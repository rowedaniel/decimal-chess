...
class Queen(Piece):
    def get_movement_spaces(self, boardSize : int, check_piece_at) -> list:
        ...
        row = self.row + 1
        col = self.col
        while row < boardSize and not check_piece_at(row, col):
        ...
        row = self.row
        col = self.col + 1
        while col < boardSize and not check_piece_at(row, col):
            spaces.append((row, col))
            col += 1
        ...
        row = self.row - 1
        col = self.col
        while row >= 0 and not check_piece_at(row, col):
            spaces.append((row, col))
            row -= 1
        ...
        row = self.row
        col = self.col - 1
        while col >= 0 and not check_piece_at(row, col):
            spaces.append((row, col))
            col -= 1
